...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse)
{
	...
	if (request.name == "forcequery" && request.queryString != "")
	{
      ...
      // TODO - do something more elegant with the response
			sendResponse({queryString: request.name, queryResults: response});
		}, function(response)
		{
		  console.log('error: ', response);
		  // TODO - do something more elegant with the response
		  sendResponse({queryString: request.name, queryResults: {}, error: true, response: response});
		});
    ...
    return true;
		...
		if (request.name == "forceObjectFields" && request.queryString != "")
	{
	
		client.describe(request.queryString, function(response)
		{
			console.log('results: ', response);
      // TODO - do something more elegant with the response
			sendResponse({queryString: request.name, queryResults: response});
